<template>
  <div class="github">
    <div class="github-box">
      ...
      <div class="tips">
        <p>
          亲爱的 {{github.login}} 您好，首次使用github账户登录，请选择一种登录方式!
        </p>
      </div>
...
<script>
  ...
  export default {
    ...
    async asyncData({store, route, params, redirect, $axios}) {
      let githubUserInfo = await $axios.get('/api/auth/github/user/userInfo', {
        params: {
      ...
      if (githubUserInfo.status !== 200) {
        ...
        // const autoRes = await $axios.post(`/api/auth/bind/gitlab/auto/login`, {
        //   id: githubUserInfo.data.id
        // })
        // console.log(autoRes)
        ...
        // console.log(autoRes)
        return {
          github: githubUserInfo.data.github,
          id: githubUserInfo.data.id
        }
      }
    },
...
//            alert('submit!');
      ...
      async bindNewUser() {
        ...
        const res = await this.$axios.post(`/api/auth/bind/gitlab/new/user`, {
          ...account,
          id: this.id
        })
        if (res.status !== 200) {
      ...
      async bindOldUser() {
        ...
        const res = await this.$axios.post(`/api/auth/bind/gitlab/old/user`, {
          ...account,
          id: this.id
        })
        if (res.status !== 200) {
    ...
    async created() {
      const autoRes = await this.$axios.post(`/api/auth/bind/gitlab/auto/login`, {
        id: this.id
      })
      if (autoRes.status === 200) {
        window.location.href = '/'
